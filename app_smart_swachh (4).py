...
MODEL_PATH = "smart_swachh_model.pt"  # Placeholder: Model not used in PIL fallback

ISSUE_CATEGORIES = ["No Issue", "Waste Issue", "Infrastructure Issue", "Safety Issue"]
CATEGORY_TO_COLOR = {
    "No Issue": "green",
    "Waste Issue": "orange",
    "Infrastructure Issue": "red",
    "Safety Issue": "blue",
}
...
st.title("\U0001F9F9 Smart Swachh - AI-Powered Civic Reporter")
...
# ----------------------------
if tab == "Citizen":
    st.header("\U0001F465 Citizen Portal")
...
# ----------------------------
    ...
    st.header("\U0001F3DB Municipal Officer Dashboard")
    if not st.session_state.reports:
        ...
        # Visual Dashboard
        st.subheader("\U0001F5FA Issue Heatmap")
        fmap = folium.Map(location=[28.6, 77.2], zoom_start=11)
        for report in st.session_state.reports:
            folium.CircleMarker(
                location=[report["lat"], report["lon"]],
                radius=8,
                popup=f"{report['category']}<br>Status: {report['status']}",
                color=CATEGORY_TO_COLOR[report["category"]],
                fill=True,
                fill_opacity=0.7
            ).add_to(fmap)
